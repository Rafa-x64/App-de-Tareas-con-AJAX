...
class usuario_model extends main_model
{
    ...
    public static function iniciarSesion($correo, $contraseña)
    {
        ...
        if (!parent::verificarContraseña($contraseña, $resultado["contraseña"])) {
            return ["ok" => false, "mensaje" => "Correo o contraseña incorrectos"];
        }

        return ["usuario" => $resultado, "ok" => true, "mensaje" => "Inicio de sesión exitoso];
    }
    ...
    public static function registrar($nombre, $apellido, $correo, $contraseña)
    {
        ...
        $correoValido = self::correoValido($correo);

        if ($correoValido["ok"] == false) {
            return ["ok" => false, "mensaje" => $correoValido["mensaje"]];
        }
    ...
    public static function correoValido($correo)
    {
        ...
        if ($resultado && $resultado["correo"] === $correo) {
            return ["ok" => false, "mensaje" => "El correo ya está asociado a una cuenta"];
        }

        return ["ok" => true, "mensaje" => "disponible"];;
    }
}
